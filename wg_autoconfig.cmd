...
echo [Interface]>>%WG_CONFIG_DIR%\client%%A.config
call echo PrivateKey = %%client_privatekey_%%A%%>>%WG_CONFIG_DIR%\client%%A.config
echo ListenPort = %CLIENT_PORT%>>%WG_CONFIG_DIR%\client%%A.config
echo Address = %CLIENT_IP_PREFIX%%%A%CLIENT_IP_MASK_CLIENT_SITE%>>%WG_CONFIG_DIR%\client%%A.config
echo [Peer]>>%WG_CONFIG_DIR%\client%%A.config
echo PublicKey = %server_publickey%>>%WG_CONFIG_DIR%\client%%A.config
call echo PresharedKey = %%client_PresharedKey_%%A%%>>%WG_CONFIG_DIR%\client%%A.config
echo AllowedIPs = %CLIENT_ROUTING%>>%WG_CONFIG_DIR%\client%%A.config
echo Endpoint = %SERVER_WAN_ADDRESS%>>%WG_CONFIG_DIR%\client%%A.config)
)
